 ...
 *
 */
public class PredictNetCollector implements INetCollector {

	private String siteContent;
	private int offset;

	public PredictNetCollector() {
		this.offset = 0;
		this.siteContent = "";
	}
	...
	public GameResult getResult() {
		...
		if (offset < siteContent.length()) {
			Pattern pRes = Pattern
					.compile(							
							"<td class=\"Heim( b)?\"><span.*?<a href=.*?>(.*?)</a></td><td class=\"vs\">-</td><td class=\"Gast.*?<a href=.*?>(.*?)</a></td>",
							Pattern.DOTALL);
			...
			if (mRes.find(offset)) {
				...
				gr = new GameResult(opp1, opp2, 0, 0, 0, 0);
